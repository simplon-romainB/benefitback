...
router.post('/',(req, res, next) =>{
    ...
    if (comparison === false) {
      ...
      const profRequete = 'INSERT INTO profil (email, ville, capital, numeroRM, numeroTva, formeJuridique, delaiPaiement, societe, conformite, association, TVA, assurance, informations, societeOrComm, TVAna, nom, id) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?, DEFAULT)'
      ...
      const request =  connection.query(profRequete, email, (err, response) => {
        res.send(JSON.stringify('profil saved'))
      })
    }
});
